...
def dance_sidebar(db, heading, text):
    ...
    full_heading = f"Ballroom/Dance Relevance \u2014 {heading} (original synthesis, not lecture content)"
    heading_lines = wrap_words(full_heading, "Lora-BoldItalic", 9.3, CW - 28)
    lines = wrap_words(text, "Lora", 8.6, CW - 32)
    box_h = 10 + len(heading_lines) * 12.5 + 6 + len(lines) * 11.8
    db.ensure(box_h + 8)
    c = db.c
    ...
    ty = db.y - 15
    for hln in heading_lines:
        c.drawString(ML + 14, ty, hln)
        ty -= 12.5
...
def embed_figure_pair(db, left_path, left_label, right_path, right_label, caption, max_h=230):
    """Embeds two images side by side (e.g. MOA internal-course + CAM
    external-point figures) with individual labels and a shared caption."""
    from reportlab.lib.utils import ImageReader
    gap = 14
    col_w = (CW - gap) / 2
    label_h = 16
    db.ensure(max_h + label_h + 20)
    c = db.c
    top = db.y
    for path, label, x in [(left_path, left_label, ML), (right_path, right_label, ML + col_w + gap)]:
        ir = ImageReader(path)
        iw, ih = ir.getSize()
        ...
        c.drawCentredString(x + col_w / 2, top - 12, label)
    db.y = top - label_h - max_h - 8
    figure_caption(db, caption)


def embed_figure_single(db, path, label, caption, max_h=280, max_w_frac=0.75):
    """Embeds a single centered image with a label and caption -- used when
    only one reference figure exists for a channel."""
    from reportlab.lib.utils import ImageReader
    label_h = 16
    max_w = CW * max_w_frac
    db.ensure(max_h + label_h + 20)
    ...
    c.drawImage(ir, dx, dy, width=dw, height=dh)
    setfill(c, NAVY); c.setFont("Lora-Bold", 9)
    c.drawCentredString(ML + CW / 2, top - 12, label)
    db.y = dy - 8
    figure_caption(db, caption)
