...
OUT = "/home/claude/ac300wk6/out/AC300_Week6_PrepGuide_Print.pdf"
...
callout_box(c, MARGIN, box_top, CONTENT_W, box_h, rgb_fill=LIGHTBLUE)
ty = box_top - 16
set_fill(c, GOLD)
...
for phase, desc in steps:
    ...
    y = draw_paragraph(c, desc, MARGIN + lbl_w, y, CONTENT_W - lbl_w, size=8.5, leading=11.5, color=BLACK)
    y -= 8

y -= 8
hairline(c, MARGIN, y, PAGE_W - MARGIN, rgb=GOLD, w=0.6)
y -= 16
...
# ============================================================= SECTION A
y = header_bar(c, "A", "Factual Recall", subtitle="Answer from memory first; check against the Study Guide only after.")
y -= 8
qA = [
...
for i, (q, lines) in enumerate(qA, 1):
    y = numbered_question(c, i, q, MARGIN, y, CONTENT_W, lines=lines + 1, gap=17)

y -= 6
box_h = 92
callout_box(c, MARGIN, y, CONTENT_W, box_h, rgb_fill=tint(MINISTER, 0.82))
ty = y - 16
...
footer(c, "PLA Sheet", 1)
c.showPage()

# ============================================================= SECTION B
y = header_bar(c, "B", "Conceptual Understanding", subtitle="Explain the logic, not just the list \u2014 this maps to the running-course quiz format.")
y -= 8
qB = [
...
for i, (q, lines) in enumerate(qB, 1):
    y = numbered_question(c, i, q, MARGIN, y, CONTENT_W, lines=lines, gap=16)

y -= 4
box_h = 60
callout_box(c, MARGIN, y, CONTENT_W, box_h, rgb_fill=LIGHTBLUE)
ty = y - 14
...
footer(c, "PLA Sheet", 2)
c.showPage()

# ============================================================= SECTION C
y = header_bar(c, "C", "Clinical Application")
y -= 8
qC = [
...
y -= 4
box_h = 50
callout_box(c, MARGIN, y, CONTENT_W, box_h, rgb_fill=tint(WOOD, 0.85))
ty = y - 14
...
footer(c, "PLA Sheet", 3)
c.showPage()

# ============================================================= SECTION D
y = header_bar(c, "D", "Synthesis & Connections")
y -= 8
qD = [
...
footer(c, "PLA Sheet", 4)
c.showPage()

# ============================================================= SECTION E
y = header_bar(c, "E", "Points of Confusion (Muddiest Point)")
y -= 8
y = draw_paragraph(c,
...
for p in prompts:
    ...
    y -= 16
    y = write_lines(c, MARGIN + 10, y, CONTENT_W - 10, n=3, gap=22)
    y -= 20
footer(c, "PLA Sheet", 5)
c.showPage()

# ============================================================= SECTION F
y = header_bar(c, "F", "Discussion / Pair-Share Prompts")
y -= 8
y = draw_paragraph(c, "For use in class with your interteaching pair or small group.",
                    MARGIN, y, CONTENT_W, size=9.5, leading=13)
y -= 16
qF = [
...
footer(c, "PLA Sheet", 6)
c.showPage()

# ============================================================= INTER-QUIZ
y = header_bar(c, None, "Inter-Quiz",
...
for acq, maint in pairs:
    tag_badge(c, MARGIN, y, "ACQ", MINISTER)
    y2 = draw_paragraph(c, acq, MARGIN + 42, y + 1, CONTENT_W - 42, size=8.8, leading=11.5)
    y = min(y2, y - 12) - 4
    tag_badge(c, MARGIN, y, "MAINT", GOLD)
    y2 = draw_paragraph(c, maint, MARGIN + 50, y + 1, CONTENT_W - 50, size=8.8, leading=11.5, color=GRAY)
    y = min(y2, y - 12) - 12
...
footer(c, "PLA Sheet \u00b7 Inter-Quiz", 7)
c.showPage()
