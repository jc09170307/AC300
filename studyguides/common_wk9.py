"""Shared rendering framework for all Week 9 (15 Collaterals, 12 Divergent
Channels, 12 Muscle/Sinew Regions, 12 Cutaneous Regions) documents. Cover
page geometry matches the locked Week 2 spec exactly. Print + reMarkable
editions controlled via sys.argv[1] in each build script."""
import sys
...
FONT_DIR = "/home/claude/ac300wk9/fonts"
FIGS_DIR = "/home/claude/ac300wk9/figs"
...
def studyguide_cover(db, title, subtitle, points_line, covers_bullets, info_lines, week_num="9"):
    c = db.c
    ...
    c.drawCentredString(bx + badge_w / 2 + 2, y - 48, week_num)
    y -= badge_w + 26
    ...
    sub_lines = wrap_words(subtitle, "Lora-BoldItalic", sub_size, CW - 20)
    for ln in sub_lines:
        c.drawCentredString(W / 2, y, ln)
        y -= sub_size + 4
    y -= 6
    setfill(c, GRAY); c.setFont("Lora-Italic", 11)
    pts_lines = wrap_words(points_line, "Lora-Italic", 11, CW - 20)
    for ln in pts_lines:
        c.drawCentredString(W / 2, y, ln)
        y -= 14
    y -= 2
    ...
    for ln in info_lines:
        wrapped_info.append(wrap_words(ln, "Lora-Italic", 8.3, CW - 28))
...
def cramsheet_cover(db, points_line, box_triplets, extras_line, week_num="9",
    ...
    c.drawCentredString(W / 2, y, ready_line)
    y -= 20
    rule_w = 240
