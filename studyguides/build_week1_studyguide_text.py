...
if IS_RM:
    ...
    OUT = "/mnt/user-data/outputs/AC300_Week1_StudyGuide_Text_reMarkable.pdf"
    EDLABEL = "reMarkable Edition"
else:
    PAGE_BG = (1, 1, 1); ROW_TINT = (0.965, 0.967, 0.972)
    HEADER_H = 44; HAIRLINE = 0.5
    OUT = "/mnt/user-data/outputs/AC300_Week1_StudyGuide_Text_Print.pdf"
...
# ============= 3 CIRCUITS DETAILED (same page) =============
y = section_rule(y, "The Three Circuits, in Full", width=220)
y = para(y, "Every one of the 12 Primary Meridians belongs to exactly one of three circuits. Each circuit "
             "contains exactly four meridians - one Hand Yin, one Hand Yang, one Foot Yang, and one Foot Yin - "
             "following the same repeating pattern: chest to hand, hand to head, head to foot, foot to chest.")
y -= 12

for circuit_name, position, members, elements in CIRCUITS:
    setfill(GOLD); c.rect(ML, y - 2, CW, 2.5, fill=1, stroke=0)
    setfill(NAVY); c.setFont("Lora-Bold", 11)
    c.drawString(ML, y - 16, f"{circuit_name} ({position})")
    setfill(GRAY); c.setFont("Lora-Italic", 9)
    c.drawRightString(ML + CW, y - 16, elements)
    y -= 30
    setfill(DARK); c.setFont("Lora", F_BODY)
    seq = "  ->  ".join(members)
...
y = para(y, "A useful way to remember: the Outer Circuit's Yang partner is Yangming (LI, ST); the Inner "
             ...
             "(SJ, GB). Match the location name to the circuit and the rest follows.", size=9.5, lh=12.5, color=GRAY)
