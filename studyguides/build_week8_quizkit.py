#!/usr/bin/env python3
"""AC300 Week 8 Quiz Kit -- standalone practice quiz for the 15 Collaterals,
12 Divergent Channels, 12 Muscle Regions, and 12 Cutaneous Regions.
MC + Luo-point matching + fill-in-blank + short answer + MAINT review
(Week 7 confluent points), with full answer key. Print + reMarkable."""
import sys
...
from common_wk8 import (DocBuilder, setfill, setstroke, box, hairline, draw_paragraph,
                         ...
                         EDLABEL)
from wk8_quiz_questions import (MC_QUESTIONS, LUO_MATCH_LEFT, LUO_MATCH_RIGHT,
                                 LUO_MATCH_ANSWER, FILL_BLANK, SHORT_ANSWER, MAINT_QUESTIONS)
from wk8_content import HOMEWORK_QUIZ_NOTE

OUT = f"/mnt/user-data/outputs/AC300_Week8_QuizKit_{'reMarkable' if IS_RM else 'Print'}.pdf"
DOC_LABEL = "Week 8 Quiz Kit"
FOOTER = "AC300/AC375 | Week 8 Quiz Kit | Collaterals, Divergent, Sinew & Cutaneous | VUIM Summer 2026"
...
c.drawString(ML, y, "Week 8 Quiz Kit")
y -= 28
setfill(c, RED); c.setFont("Lora-Bold", 15.5)
c.drawString(ML, y, "Collaterals \u00b7 Divergent Channels \u00b7 Muscle Regions \u00b7 Cutaneous Regions")
y -= 22
setfill(c, GRAY); c.setFont("Lora-Italic", 11)
c.drawString(ML, y, "Standalone practice quiz -- pairs with the Week 8 Study Guide")
y -= 18
...
bullets = [
    f"{len(MC_QUESTIONS)}-Question MCQ Section \u2014 Luo points, Divergent Channel structure, Muscle Regions, Cutaneous Regions",
    "Luo-Connecting Point Matching \u2014 10 meridians matched to their Luo points",
    "Fill-in-the-Blank \u2014 the 3 extra collaterals, Li-He-Chu-merge terms, structural pattern rules",
    "Short Answer \u2014 the \u201cwhy 15\u201d logic, special-point overlaps, and clinical application",
    "MAINT Review \u2014 Week 7 confluent points, from Dr. Zhang's live review at the start of Lecture 8",
...
c.drawString(ML + 16, y, "This kit is self-test review material, not a stand-in for a graded quiz this week.")
y -= 30
...
# ============================================================
db.new_page()
y = title_bar("Section B: Luo-Connecting Point Matching", "Match each meridian to its Luo point")
y -= 10
setfill(c, DARK); c.setFont("Lora-Italic", 9)
c.drawString(ML, y, "Match each meridian (left) to its Luo-Connecting point (right). Each letter is used exactly once.")
y -= 22
col_w = CW / 2 - 10
for i, left in enumerate(LUO_MATCH_LEFT):
    ly = y - i * 20
...
right_x = ML + col_w + 20
for i, right in enumerate(LUO_MATCH_RIGHT):
    ly = y - i * 20
...
c.drawString(ML, y, "Bonus: for any 2 points above, name the region their Luo points cluster around (wrist vs. ankle).")
db.end_page()
...
# ============================================================
# SECTION E -- MAINT Review (Week 7 confluent points)
# ============================================================
db.new_page()
y = title_bar("Section E: MAINT Review \u2014 Week 7 Confluent Points", "From Dr. Zhang's live Lecture 8 review")
y -= 14
...
c.drawString(ML, y, "Section B: Luo-Connecting Point Matching")
y -= 16
setfill(c, DARK); c.setFont("Lora", 9)
match_line = "  ".join(f"{k}-{v}" for k, v in LUO_MATCH_ANSWER.items())
