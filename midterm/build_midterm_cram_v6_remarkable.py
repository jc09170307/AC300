#!/usr/bin/env python3
"""AC300 Midterm Cram Sheet (Weeks 1-4 cumulative) - reMarkable edition
Forked from build_midterm_cram_v6.py (Print). Structural deltas per project standard:
ivory page background, taller header bars, bolder hairlines, tinted parchment
row backgrounds, double gold rule on cover."""
import math
...
ROW_TINT = (0.925, 0.902, 0.855)       # distinctly tinted parchment row background (vs near-white in Print)
CALLOUT_TINT = (0.918, 0.886, 0.816)   # deeper parchment for callout boxes so they stay visible against IVORY page bg (plain CREAM is nearly identical to IVORY)
...
def header(subtitle):
    ...
    c.drawString(36, H-33, "AC300 MIDTERM CRAM SHEET")
...
setstroke(GOLD); c.setLineWidth(1.2*LW_MULT); c.line(36, y, 220, y); y -= 16
...
def circuit_box(x, y, w, h, title, top_pair, bot_pair, covered_top, covered_bot):
    ...
    setstroke(GOLD); c.setLineWidth(1*LW_MULT); c.roundRect(x, y-h, w, h, 6, fill=0, stroke=1)
    ...
    for pair, covered in [(top_pair, covered_top), (bot_pair, covered_bot)]:
        ...
        setstroke(fill_col if covered else MUTED); c.setLineWidth(1.6*LW_MULT)
...
setstroke(GOLD); c.setLineWidth(1.2*LW_MULT); c.line(36, y, 280, y); y -= 12
...
for i,(cat, meaning, ex) in enumerate(legend_rows):
    ...
    y -= row_h
    setstroke(LIGHTGRAY); c.setLineWidth(0.5*LW_MULT)
    c.line(36, y, W-36, y)
...
def pathway_diagram(x, y, w, h, title, color, waypoints, organs, note=None):
    ...
    setfill((0.973,0.974,0.978)); c.roundRect(x, y-h, w, h, 6, fill=1, stroke=0)
    setstroke(color); c.setLineWidth(1.3*LW_MULT); c.roundRect(x, y-h, w, h, 6, fill=0, stroke=1)
    ...
    c.setDash(3, 2); setstroke(GRAY); c.setLineWidth(1.2*LW_MULT)
    ...
    setstroke(color); c.setLineWidth(2*LW_MULT)
    ...
    setstroke((0.85,0.85,0.85)); c.setLineWidth(0.7*LW_MULT)
