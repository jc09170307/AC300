#!/usr/bin/env python3
"""AC300 Practice Midterm (Weeks 1-4 cumulative) - builds BOTH Print and reMarkable editions."""
import sys
...
from midterm_questions_set2 import SECTIONS, TOTAL_Q
...
SECTION_ACCENTS = {
    "Week 1": NAVY, "Week 2": METAL, "Week 3": EARTH, "Week 4": FIRE, "Weeks 1-4": RED,
}
...
if IS_RM:
    PAGE_BG = (0.973, 0.953, 0.902)          # warm ivory
    ROW_TINT = (0.925, 0.902, 0.855)          # tinted parchment rows
    CALLOUT_TINT = (0.918, 0.886, 0.816)
    HEADER_H = 51
    COVER_MASTHEAD_H = 86
    HAIRLINE = 1.0
    WRITE_FILL = (0.973, 0.953, 0.902)        # parchment (matches bg, not pure white)
    OUT = "/mnt/user-data/outputs/AC300_PracticeMidterm2_Wk1-4_reMarkable.pdf"
    EDLABEL = "reMarkable Edition"
else:
    PAGE_BG = (1, 1, 1)
    ROW_TINT = (0.965, 0.967, 0.972)
    CALLOUT_TINT = (0.961, 0.941, 0.918)      # CREAM
    HEADER_H = 44
    COVER_MASTHEAD_H = 80
    HAIRLINE = 0.5
    WRITE_FILL = (1, 1, 1)
    OUT = "/mnt/user-data/outputs/AC300_PracticeMidterm2_Wk1-4_Print.pdf"
...
def header(subtitle):
    ...
    c.drawString(36, H - HEADER_H + 15, "AC300 PRACTICE MIDTERM - SET 2")
...
def footer(page_label):
    ...
    c.drawCentredString(W / 2, 22, f"AC300/AC375 Practice Midterm Set 2 (Wk 1-4)  \u00b7  VUIM Summer 2026  \u00b7  {page_label}")
...
if IS_RM:
    ...
    c.rect(0, H - COVER_MASTHEAD_H - 5, W, 2, fill=1, stroke=0)  # double gold rule
else:
    c.rect(0, H - COVER_MASTHEAD_H, W, 3, fill=1, stroke=0)
...
c.drawCentredString(W / 2, H - 227, "PRACTICE MIDTERM — SET 2")
...
c.drawCentredString(W / 2, H - 268, f"{TOTAL_Q} questions  \u00b7  MC / True-False / EXCEPT format  \u00b7  Weeks 1-4  \u00b7  Quiz Ready")
...
labels = [
    ...
    ("COVERAGE", "Theory + 6 channels", "125 points cumulative", (0.380, 0.180, 0.522)),
    ("ANSWER KEY", "Full explanations", "for all 28 questions", (0.106, 0.369, 0.353)),
]
...
howto = [
    "Close the book. No notes, no app - simulate real exam conditions.",
    "Take all 28 in one sitting (Dr. Zhang's format: ~20-30 questions, timed).",
    "Score yourself, then re-drill ONLY the categories you missed.",
    "This is a SECOND, different set of 28 questions - no overlap with Practice Midterm Set 1.",
    "Several questions below are pulled directly from Dr. Zhang's own Quiz 1-3 answer-key reviews.",
]
yy = by0 - 42
for line in howto:
...
new_page("Questions 1-28  \u00b7  closed book")
y = H - HEADER_H - 22
...
# If the last question page has generous room left, fill it with a
# useful scoring guide rather than leaving it mostly blank.
if y > 300:
    y -= 10
    setstroke(GOLD); c.setLineWidth(HAIRLINE * 1.2)
    c.line(ML, y, ML + CW, y)
    ...
    y -= 18
    scoring = [
        ("25-28 correct", "Exam ready. Do a final pass on the Special Points Decoder for each channel."),
        ("21-24 correct", "Solid. Re-drill the specific sections you missed (see section headers above)."),
        ("16-20 correct", "Re-read the Cumulative Cram Sheet and Study Guide before the midterm."),
        ("Below 16", "Full re-study needed - work back through each week's Study Guide in order."),
    ]
    ...
    c.drawString(ML, y, "Note: several ST/SP questions above are adapted from your verified Week 3 Quiz Kit, since Dr. Zhang")
    y -= 11
    c.drawString(ML, y, "confirmed the midterm reuses a number of quiz questions - review that kit's Set A/B once more too.")
...
def answer_block(qd, sec_title):
    global y
    ans_letter = qd['ans']
    prefix_w = 38  # indent width reserved for "Qn  X" prefix
...
for sec_title, sec_wk, questions in SECTIONS:
    ...
    setfill(SECTION_ACCENTS[sec_wk]); c.rect(ML, y - 2, CW, 2.5, fill=1, stroke=0)
    setfill(NAVY); c.setFont("Lora-Bold", 10)
    c.drawString(ML, y - 14, f"{sec_title}  ({sec_wk})")
    y -= 24
    for qd in questions:
        answer_block(qd, sec_title)
